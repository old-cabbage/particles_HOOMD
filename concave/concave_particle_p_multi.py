...
def task(n):
    ...
    #粒子总数
    num_particles = list(range(1000*(n%5)+200,1000*(n%5)+1200,200))
    #每次开始插入之前，先对基础的系统预处理pre_random步数
    pre_random = 2000
    #粒子的总面积/盒子面积为packing_density
    #packing_density_0=0.5
    packing_density_0=0.1*(n//5+1)
    ...
    for j in num_particles:
        ...
        system.simulation = hoomd.Simulation(device=device,seed=random.randint(1,30))
        system.simulation.operations.integrator = mc
        system.simulation.create_state_from_gsd(filename='./gsd_concave/P_concave_{:.2f}_{}.gsd'.format(packing_density_0,j))

        # 运行循环模拟和插入测试
        iterations = 400            # 循环次数
        moves_per_cycle = 5            # 每个循环中移动的成功步数
        insertions_per_cycle = 200    # 每个循环中插入的粒子尝试次数
        ...
        for cycle in range(1, iterations + 1):
            ...
            if cycle % (iterations // 10 ) == 0:
                ...
                print(f"堆叠密度为{packing_density_0:.1f}，粒子数为{j}的循环 {cycle}/{iterations}: 成功插入 {success}/{insertions_per_cycle} 个粒子;耗时: {simulation_interval_time - simulation_start_time:.2f} 秒")
        
        simulation_end_time = time.time()
        print(f"\n堆叠密度为{packing_density_0:.1f}，粒子数为{j}的循环模拟和插入测试完成，耗时: {simulation_end_time - simulation_start_time:.2f} 秒")
        # 计算最终的成功插入概率
        ...
        with open('./result/gsd_concave_{:.2f}/concave_result.txt'.format(packing_density_0),'a') as file:
            file.write(f"\n堆叠密度为{packing_density_0:.1f}，粒子数为{j}的循环模拟和插入测试完成，耗时: {simulation_end_time - simulation_start_time:.2f} 秒\n")
